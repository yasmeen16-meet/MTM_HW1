...
static MapResult editData(Map* map, Node* iterator ,MapKeyElement keyElement,  MapDataElement dataElement);
static MapResult addKey(Map map ,MapKeyElement key ,MapDataElement data);
...
///mapCopy function -(addOrDestroy / addAllOrDestroy):
static MapResult addOrDestroy(Map map, MapKeyElement keyElement, MapDataElement dataElement) {
...
///mapPut function ( nodeCreate editData addKey);
static Node nodeCreate (Map map ,MapKeyElement keyElement,  MapDataElement dataElement) {
...
static MapResult addKey(Map map ,MapKeyElement key ,MapDataElement data) {
    Node node = nodeCreate(map, key, data);
    if (node == NULL) {
        return MAP_OUT_OF_MEMORY;
    }
    if (map->size == 0) {
...
MapResult mapPut(Map map, MapKeyElement keyElement, MapDataElement dataElement) {
    ...
    if (mapContains(map, keyElement) == true) {
        mapGetFirst(map);
        MapResult result = editData(&map, &map->iterator, keyElement, dataElement);
        mapGetFirst(map);
        return result;
    }
    MapResult result = addKey(map, keyElement, dataElement);
